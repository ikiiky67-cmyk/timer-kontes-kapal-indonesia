...
export default function HistoryDetailDrawer({ isOpen, onClose, team }: HistoryDetailDrawerProps) {
  ...
  const exportToPDF = () => {
    ...
    doc.text(`Date of Export: ${exportDate}`, 196, 22, { align: "right" });

    // Team Identity
    doc.setFontSize(16);
    doc.setFont("helvetica", "bold");
    doc.setTextColor(15, 23, 42); // slate-900
    doc.text(team.name, 14, 34);

    doc.setFontSize(12);
    doc.setFont("helvetica", "normal");
    doc.setTextColor(71, 85, 105); // slate-600
    doc.text(`Division: ${team.division} | Institution: ${team.institution}`, 14, 40);
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text(
        `Generated by Competition Timer System - Page ${i} of ${pageCount}`,
