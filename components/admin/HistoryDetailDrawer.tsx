...
export default function HistoryDetailDrawer({ isOpen, onClose, team }: HistoryDetailDrawerProps) {
  ...
  const exportToPDF = () => {
    ...
    doc.text(`Generated on: ${exportDate}`, pageWidth - 14, 22, { align: "right" });

    // Team Identity
    doc.setFontSize(16);
    doc.setFont("helvetica", "bold");
    doc.setTextColor(30, 64, 175); // blue-800
    doc.text(`Team: ${team.name}`, 14, 34);

    doc.setFontSize(11);
    doc.setFont("helvetica", "normal");
    doc.setTextColor(71, 85, 105); // slate-600
    doc.text(`Division: ${team.division}   |   Institution: ${team.institution}`, 14, 41);
    ...
    autoTable(doc, {
      ...
      columnStyles: {
        ...
        5: { halign: 'center' }, // flexible width
      },
    });
    ...
    // Add new page if space is too tight for signature
    ...
    doc.text(`Pekanbaru, ${dateStr}`, pageWidth - 14, signatureY, { align: 'right' });
    
    doc.text("( Chief Referee / Head Judge )", pageWidth - 14, signatureY + 30, { align: 'right' });
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text(
        `Generated by KKI Timer System - Page ${i} of ${pageCount}`,
        pageWidth / 2,
